...
def _classify_exit(opp, last_price: float) -> tuple[str, str, float] | None:
    ...
    (In a single-bar check, we can only know the day's high/low implicitly
    from the close. The bot handles intraday checks; we handle EOD here.)
    """
    today  = date.today()
    ...
    with app.app_context():
        try:
            ...
            for opp in pending:
                ...
                closed += 1
                logger.info(
                    "outcome_job: %s → %s (%s, pnl=%.2f%%, hold=%sd)",
                    sym, outcome, exit_reason, pnl_pct, hold_days,
                )
